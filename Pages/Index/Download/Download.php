<style media="screen">
  .downloadApp{
    background: #e5e4e2;
    padding: 40px 30px;
    position: relative;
  }
  .downloadApp h2{
    color: #062537;
  }
  .downloadApp p{
    color: #062537;
    text-align: center;
  }
  .downloadAppContainer{
    position: relative;
    height: 100%;
    width: 85vw;
    display: flex;
    max-width: 1000px;
    left: 50%;
    transform: translateX(-50%);
    margin: 50px 0px;
    flex-wrap: wrap;
    justify-content: center;

  }
  .appContainerBox{
    height: 100%;
    width: 46%; /*Pendiente por arreglar*/
    min-width: 300px;
    position: relative;
    padding: 0px 15px;
    position: relative;
  }
  .appContainerBox img{
    position: relative;
    height: 80px;
    left: 50%;
    transform: translateX(-50%);
    margin-bottom: 20px;
  }
  .containerPDownload{
    position: relative;
    width: 300px;
    left: 50%;
    transform: translateX(-50%);
  }
  .appContainerBox p{
    text-align: left;
  }
</style>
<section class="downloadApp">
  <h2>Descarga nuestra App  a partir del 22 de septiembre</h2>
  <p> y sigue los siguientes pasos para instalarla</p>
  <div class="downloadAppContainer">
    <div class="appContainerBox">
      <a href="https://icedrive.net/s/ZRARjYPYj7kXS675BSwQtjaX7uz7"><img src="Pages/Index/Download/Win.png" alt=""></a>
      
      <div class="containerPDownload">
        <p>1. Descarga la aplicacion para Windows</p>

        <p>2. Descomprime el archivo</p>

        <p>3. Doble-click en impresistem.exe</p>
      </div>

    </div>
    <div class="appContainerBox">
      <a href="https://icedrive.net/s/ZRARjYPYj7kXS675BSwQtjaX7uz7"><img src="Pages/Index/Download/Mac.png" alt=""></a>
      <div class="containerPDownload">
        <p>1. Descarga la aplicacion para Mac</p>

        <p>2. Descomprime el archivo</p>

        <p>3. Doble-click en impresistem.app</p>

      </div>

    </div>


  </div>
</section>
<script type="text/javascript">

</script>
